...
export function ReviewsTable({ initialData, locale }: { initialData: Review[], locale: string }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex bg-gray-100 dark:bg-slate-800 p-1.5 rounded-[1.5rem] w-fit border border-gray-200 dark:border-slate-700 shadow-inner">
        {["PENDING", "APPROVED", "REJECTED"].map((s) => (
          <button
            ...
            className={cn(
              ...
              filter === s 
                ? "bg-white dark:bg-slate-900 text-brand-blue shadow-md dark:shadow-none scale-[1.02]" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredData.map((review) => (
          <div key={review.id} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-gray-100 dark:border-slate-800 shadow-sm dark:shadow-none flex flex-col hover:shadow-lg transition-all group overflow-hidden relative">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 rounded-2xl bg-brand-blue/5 dark:bg-brand-blue/10 flex items-center justify-center text-brand-blue text-lg font-bold">
                {review.guestName.charAt(0)}
              </div>
              <div>
                <p className="font-bold text-gray-900 dark:text-gray-100 leading-tight">{review.guestName}</p>
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-8 flex-1 italic">
              "{review.comment}"
            </p>
